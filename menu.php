				...
				<form action="" id="manage-menu">
					<div class="card">
						...
						<div class="card-body">
							...
							<div class="form-group">
								<img src="<?= isset($image_path) ? '../assets/img/' . $cover_img : '' ?>" alt="" id="cimg">
							</div>
						</div>
						...
						<div class="card-footer">
							<div class="row">
								<div class="col-md-12">
									...
									<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="Rs('#manage-menu').get(0).reset()"> Cancel</button>
								</div>
							</div>
						</div>
					</div>
								...
								?>
									<tr>
										...
										<td class="text-center">
											<img src="<?= isset($row['img_path']) ? '../assets/img/' . $row['img_path'] : '' ?>" alt="" id="cimg">
										</td>
										<td class="">
											<p>Name : <b><?= $row['name'] ?></b></p>
											<p>Description : <b class="truncate"><?= $row['description'] ?></b></p>
											<p>Price : <b><?= "Rs." . number_format($row['price'], 2) ?></b></p>
										</td>
										<td class="text-center">
											<button class="btn btn-sm btn-primary edit_menu" type="button" data-id="<?= $row['id'] ?>" data-name="<?= $row['name'] ?>" data-status="<?= $row['status'] ?>" data-description="<?= $row['description'] ?>" data-price="<?= $row['price'] ?>" data-img_path="<?= $row['img_path'] ?>">Edit</button>
											<button class="btn btn-sm btn-danger delete_menu" type="button" data-id="<?= $row['id'] ?>">Delete</button>
										</td>
									</tr>
								<?php endwhile; ?>
							</tbody>
