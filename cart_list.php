 			...
 			<div class="col-lg-8">
 				...
 				<?php
					if (isset($_SESSION['login_user_id'])) {
						...
						$ip = isset($_SERVER['HTTP_CLIENT_IP']) ? $_SERVER['HTTP_CLIENT_IP'] :  $_SERVER['REMOTE_ADDR'] ; isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'] ;
 						...
 						<div class="card-body">
 							<div class="row">
 								<div class="col-md-4" style="text-align: -webkit-center">
 									<a href="admin/ajax.php?action=delete_cart&id=<?= $row['cid'] ?>" class="rem_cart btn btn-sm btn-outline-danger" data-id="<?= $row['cid'] ?>"><i class="fa fa-trash"></i></a>
 									<img src="assets/img/<?= $row['img_path'] ?>" alt="">
 								</div>
 								<div class="col-md-4">
 									<p><b>
 											<large><?= $row['name'] ?></large>
 										</b></p>
 									<p class='truncate'> <b><small>Desc :<?= $row['description'] ?></small></b></p>
 									<p> <b><small>Unit Price :<?= number_format($row['price'], 2) ?></small></b></p>
 									<p><small>QTY :</small></p>
 									<div class="input-group mb-3">
 										<div class="input-group-prepend">
 											<button class="btn btn-outline-secondary qty-minus" type="button" data-id="<?= $row['cid'] ?>"><span class="fa fa-minus"></button>
 										</div>
 										<input type="number" readonly value="<?= $row['qty'] ?>" min=1 class="form-control text-center" name="qty">
 										<div class="input-group-prepend">
 											<button class="btn btn-outline-secondary qty-plus" type="button" id="" data-id="<?= $row['cid'] ?>"><span class="fa fa-plus"></span></button>
 										</div>
 									</div>
 								</div>
 								<div class="col-md-4 text-right">
 									<b>
 										<large><?= number_format($row['qty'] * $row['price'], 2) ?></large>
 									</b>
 								</div>
 							</div>
 						</div>
 				...
 				<div class="sticky">
 					<div class="card">
 						<div class="card-body">
 							...
 							<hr>
 							<p class="text-right"><b><?= number_format($total, 2) ?></b></p>
 							<hr>
